# Aegis skill — offline (bundled)

A downloadable [Agent Skill](https://aegismcp.io) that lets a Claude-style agent scan a token
for safety before you buy. This is the **offline** build: Aegis `0.4.1` and all of its
...
3. Install the scanner from the bundled wheels (run from the `aegis/` folder):

       pip install --no-index --find-links wheels aegis

4. Restart / reload the agent. It reads `SKILL.md` and uses Aegis when you ask about token safety.
...
- Aegis `0.4.1` + every dependency, as wheels.
...
- **Thin:** a few KB, always installs the latest from PyPI, needs internet. Get it at https://aegismcp.io
